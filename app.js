...
/* AJAYNXT V14 Superdesign integration */
(() => {
  ...
  const up = [...document.querySelectorAll('.parallax-card-up')];
  const down = [...document.querySelectorAll('.parallax-card-down')];
  let raf = 0;
  const render = () => {
    raf = 0;
    const y = window.scrollY || 0;
    if (!reduced.matches && heroContent && y < 950) {
      heroContent.style.transform = `translate3d(0, ${Math.min(160, y * .16)}px, 0)`;
    ...
    if (!reduced.matches) {
      ...
      [...up, ...down].forEach((el) => {
        ...
        const centered = (r.top + r.height / 2 - viewport / 2) / viewport;
        const amount = Math.max(-1, Math.min(1, centered)) * 34;
        el.style.transform = `translate3d(0, ${up.includes(el) ? amount : -amount}px, 0)`;
      });
    }
  };
...
/* AJAYNXT V15 neural hero */
(() => {
  const media = document.querySelector('[data-hero-media]');
  const hero = document.querySelector('.nx-hero');
  const reduce = matchMedia('(prefers-reduced-motion: reduce)');
  const fine = matchMedia('(hover:hover) and (pointer:fine)');
  if (!media || !hero || reduce.matches || !fine.matches) return;
  let tx = 0, ty = 0, x = 0, y = 0, raf = 0;
  const animate = () => {
    x += (tx - x) * .07;
    y += (ty - y) * .07;
    media.style.transform = `translate3d(${x}px, ${y}px, 0) scale(1.025)`;
    if (Math.abs(tx-x) > .05 || Math.abs(ty-y) > .05) raf = requestAnimationFrame(animate); else raf = 0;
  };
